...
namespace NSMBe5
{
	public class NSMBStageObj : LevelItem
	{
		public int X;
		public int Y;
		public int Type;
		...
		public NSMBStageObj(NSMBLevel Level)
		{
			this.Level = Level;
		}

		public NSMBStageObj(NSMBStageObj s)
		{
			this.X = s.X;
			this.Y = s.Y;
			this.Type = s.Type;
		...
		private static int[] AlwaysDrawNums = { 68, 69, 73, 141, 226, 305, 306, /* and now, featuring: */ 198, 231, 234, 259, 276 };
		...
		public void Render(Graphics g, LevelEditorControl ed)
		{
			...
			switch (ObjectID)
			{
				...
				case 21:
					img = new Bitmap(Properties.Resources.mario_small, new Size(24, 24));
					img2 = new Bitmap(Properties.Resources.luigi_small, new Size(24, 24));
					Bitmap invalid = new Bitmap(Properties.Resources.sprite_invalid, new Size(24, 24));

					//Draw clone character
					byte character = (byte)((Data[5] & 0xF0) / 16);
					switch (character)
					{
						case 0:
							g.DrawImage(img, RenderX - 12, RenderY - 24);
							break;
						case 1:
							g.DrawImage(img2, RenderX - 12, RenderY - 24);
							break;
						default:
							g.DrawImage(invalid, RenderX - 12, RenderY - 24);
							if (character < 10)
								g.DrawString(character.ToString(), NSMBGraphics.SmallInfoFont, Brushes.White, RenderX - 6, RenderY - 20);
							else
								g.DrawString(character.ToString(), NSMBGraphics.SmallInfoFont, Brushes.White, RenderX - 10, RenderY - 20);
							break;
					}

					//Draw controller
					byte controller = (byte)(Data[5] & 0x0F);
					switch (controller)
					{
						case 0:
							g.DrawImage(img, RenderX + 5, RenderY - 12, 12, 12);
							break;
						case 1:
							g.DrawImage(img2, RenderX + 5, RenderY - 12, 12, 12);
							break;
						default:
							g.DrawString(controller.ToString(), NSMBGraphics.SmallInfoFont, Brushes.White, RenderX + 8, RenderY - 16);
							break;
					}
					break;
				case 23:
				...
				case 274: // Bottom left vertical camera limit
				{
					...
					foreach (var obj in Level.Sprites)
					{
						if (obj.Type != 199 || obj.Y != Y || obj.X <= X)
							continue;
					...
					{
						int length = (closestX - X) * 16 + 16;
						Pen pen = new Pen(Color.Maroon, 4);
						pen.DashStyle = DashStyle.Dash;
						pen.DashPattern = new float[2] { 2.0f, 2.0f };
						g.DrawLine(
							pen,
							new Point(RenderX, RenderY),
							new Point(RenderX + length, RenderY)
						);
					}
				...
				case 277: // Horizontal camera limit
				{
					Color a = Color.White;
					Color b = Color.FromArgb(0, Color.White);

					bool left = (Data[5] & 1) != 0;
					int heightHi = Data[4] & 0x0F;
					int heightLo = Data[5] & 0xF0;

					int lineHeight = 16 * ((heightHi << 4) | (heightLo >> 4));
					int lineWidth = 16;

					int x1 = RenderX;
					int x2 = RenderX + lineWidth;
					int y1 = RenderY - lineHeight;
					int y2 = RenderY + 16;

					if (!left)
						(a, b) = (b, a);

					using (LinearGradientBrush brush = new LinearGradientBrush(new Point(x1, 0), new Point(x2, 0), a, b))
					{
		...
		private Bitmap GetLiquidTexture()
		{
			switch (ObjectID)
			{
				...
				case 279:
					return Properties.Resources.liquid_lava_0;
				case 281:
					return Properties.Resources.liquid_poison_0;
				default:
					return null;
			}
		}
		...
		public static NSMBStageObj FromString(string[] strs, ref int idx, NSMBLevel lvl)
		{
			NSMBStageObj s = new NSMBStageObj(lvl);
