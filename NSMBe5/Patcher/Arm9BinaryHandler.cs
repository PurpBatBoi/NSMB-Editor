...
namespace NSMBe5.Patcher
{
    public class Arm9BinaryHandler
    {
        ...
        {
            if (isCompressed)
                new ErrorMSGBox("", "", "", "Could not load sections of compressed ARM9!").ShowDialog();
        ...
        public void writeToRamAddr(int ramAddr, uint val, int ov)
        {
			if(ov != -1)
			{
		        foreach(Overlay of in ROM.arm9ovs)
		        	if(of.ovId == ov)
		        	{
                        ...
                        {
                            new ErrorMSGBox("", "", "", "WRITE: Overlay ID " + ov + " doesn't contain address:\n0x" + ramAddr.ToString("X4")).ShowDialog();
                            return;
                            //throw new Exception("WRITE: Overlay ID "+ov+" doesn't contain addr "+ramAddr+" :(");
                        }

                    }

                new ErrorMSGBox("", "", "", "WRITE: Overlay ID " + ov + " not found!").ShowDialog();
                return;
            ...
            new ErrorMSGBox("", "", "", "WRITE: Address 0x" + ramAddr.ToString("X4") + " is not in ARM9 binary or overlays!").ShowDialog();
            return;
        ...
        public uint readFromRamAddr(int ramAddr, int ov)
        {
			if(ov != -1)
			{
		        foreach(Overlay of in ROM.arm9ovs)
		        	if(of.ovId == ov)
		        	{
                        ...
                        {
                            new ErrorMSGBox("", "", "", "READ: Overlay ID " + ov + " doesn't contain address\n0x" + ramAddr.ToString("X4")).ShowDialog();
                            return 0;
                            //throw new Exception("READ: Overlay ID "+ov+" doesn't contain addr "+ramAddr+" :(");
                        }
                    }

                new ErrorMSGBox("", "", "", "READ: Overlay ID " + ov + " not found!").ShowDialog();
                return 0;
			...
			else
			{
                ...
                new ErrorMSGBox("", "", "", "READ: Address 0x" + ramAddr.ToString("X4") + " is not in arm9 binary or overlays!").ShowDialog();
                return 0;
